...
function wrap(index: number, max: number): number {
...
export function countLiveNeighbors(
  ...
  row: number,
  col: number,
  options: SimulationOptions = { wrappingEdges: false },
): number {
  ...
  for (let r = -1; r <= 1; r++) {
    for (let c = -1; c <= 1; c++) {
      ...
      if (options.wrappingEdges) {
        nextRow = wrap(nextRow, rows);
        nextCol = wrap(nextCol, cols);
...
export function nextGeneration(
  ...
  return grid.map((row, rowIndex) =>
    row.map((isAlive, colIndex) => {
      const liveNeighbors = countLiveNeighbors(grid, rowIndex, colIndex, options);
...
export function deserializeGrid(serialized: SerializedGrid): Grid {
  const grid = createEmptyGrid(serialized.rows, serialized.cols);

  for (const [row, col] of serialized.cells) {
    if (row >= 0 && row < serialized.rows && col >= 0 && col < serialized.cols) {
      grid[row][col] = true;
    }
  }
